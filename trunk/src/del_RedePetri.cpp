...
/* O ( 1 )  */
void RedePetri::Novo_Arco_PT( int de , int para , int custo)
{
    if(!P.existeElemento(de) || !T.existeElemento(para))
	{
...
void RedePetri::Novo_Arco_TP( int de , int para , int custo)
{
    if(!T.existeElemento(de) || !P.existeElemento(para))
	{
		printf("Erro, Novo Arco invalido, Transicao ou Lugar inexistente\n" );
...
void RedePetri::Executa_Transicao(int t)
{
	if( Sensibilizado(t) )
	{
		M = M + C.coluna(t);
 ...
 */
int RedePetri::Numero_Vertices()
{
    return V.tamanho();
}

int RedePetri::Numero_Transicoes()
{
    return T.tamanho();
}
		
int RedePetri::Numero_Lugares()
{
    return P.tamanho();
}

int RedePetri::Numero_Arcos()
{
    return N_Arcos;
}

int RedePetri::Numero_Fichas(int p)
{
    if(!P.existeElemento(p))
    {
...
int RedePetri::Precedente_T(int t)
{
    if(T.existeElemento(t))
	{
...
/* Retorna ID do precedente ou -1 se não houver precedente */
int RedePetri::Precedente_P(int p)
{
    if(P.existeElemento(p))
	{
...
int RedePetri::Precedente(int gID)
{
	int id;
    if(V.existeElemento(gID))
    {
...
const Lugar &RedePetri::lugar(int id)
{
 ...
 */
int RedePetri::Custo_PT(unsigned p, unsigned t)
{
 ...
 * @return int - Custo do arco (Numero de fichas)
 */
int RedePetri::Custo_TP(unsigned t, unsigned p)
{
    if(P.existeElemento(p) && T.existeElemento(t))
    {
        return Pos(p,t);
